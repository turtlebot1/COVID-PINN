%% ================== SLIR PINN over 50×50 grid, fitting SIR data ==================
clear; clc; close all
rng(1)
...
t_norm = (days - 1) / (max(days)-1);  % normalize to [0,1]

%% ------------------------ Physics parameters ----------------------------
% params.beta   = 1.0;     
% params.sigma  = 0.6;     % latent -> infectious rate
% params.gamma  = 0.5;     
...
params.DI     = 0.0062;
params.DR     = 5.3e-8;
params.lambda = 0.68 ;   % recruitment/birth rate
params.theta  = 2.82e-5 ;   % natural death/removal rate
params.phi    = 4.69e-4;   % infection rate
params.delta  = 1.0e-5;   % removal rate of I
params.omega  = 0.0517;   % recovery boosting R
params.sigma = 0.5;

params.tmin   = 1; params.tmax = 45;

params.lambda_data = 10.0;   % supervised data weight
params.lambda_pde  = 1.0;    % physics residual weight
...
for k = 1:numel(t_norm)
    ...
    % Add non-uniform initialization
...
layers = [layers; fullyConnectedLayer(4)]; % outputs [S,L,I,R]
net = dlnetwork(layers);

%% ------------------------- Loss function --------------------------------
function [loss, gradients] = slirLoss2D_noLdata(net, Xdl, Ydl, Tdl, Sd_dl, Id_dl, Rd_dl, params)
    XYT = cat(1,Xdl,Ydl,Tdl);
    U   = forward(net,XYT);
    S = U(1,:); L = U(2,:); I = U(3,:); R = U(4,:);
    ...
    % PDE residuals
    % fS = ts*St - params.DS*(Sxx+Syy) + params.beta.*S.*I;
    % fL = ts*Lt - params.DL*(Lxx+Lyy) - params.beta.*S.*I + params.sigma.*L;
    % fI = ts*It - params.DI*(Ixx+Iyy) - params.sigma.*L + params.gamma.*I;
    % fR = ts*Rt - params.DR*(Rxx+Ryy) - params.gamma.*I;

    % PDE residuals (scaled system with sigma*L instead of convolution)
...
for ep = 1:numEpochs
    [loss,gradients] = dlfeval(@slirLoss2D_noLdata,net,Xd_dl,Yd_dl,Td_dl,Sd_dl,Id_dl,Rd_dl,params);
...
for k = 1:numel(days_all)
    ...
    [Xint,Yint] = meshgrid(xg(ix_in),yg(iy_in));
    XYT = [Xint(:)'; Yint(:)'; t_norm_k*ones(1,numel(Xint))];
...
for d = daysToPlot
    ...
    Smap_true(iy_in,ix_in) = S_data(d);   % uniform true values
    ...
    nexttile; imagesc(xg,yg,Smap_true,'AlphaData',~isnan(Smap_true)); axis image; colorbar
    title('S True'); set(gca,'YDir','normal');
    nexttile; imagesc(xg,yg,Smap_pred,'AlphaData',~isnan(Smap_pred)); axis image; colorbar
    title('S PINN'); set(gca,'YDir','normal');

    nexttile; imagesc(xg,yg,Imap_true,'AlphaData',~isnan(Imap_true)); axis image; colorbar
    title('I True'); set(gca,'YDir','normal');
    nexttile; imagesc(xg,yg,Imap_pred,'AlphaData',~isnan(Imap_pred)); axis image; colorbar
    title('I PINN'); set(gca,'YDir','normal');

    nexttile; imagesc(xg,yg,Rmap_true,'AlphaData',~isnan(Rmap_true)); axis image; colorbar
    title('R True'); set(gca,'YDir','normal');
    nexttile; imagesc(xg,yg,Rmap_pred,'AlphaData',~isnan(Rmap_pred)); axis image; colorbar
    title('R PINN'); set(gca,'YDir','normal');
end
