...
I_true(1,round(Nx/2),round(Ny/2)) = 100; % infection bump at center
dt = 0.1;
for k = 2:Nt
...
function [loss,grads, Ybd, Yic, S0, I0, R0, fS, fI, fR, test1, test2, test3, test4] = modelLoss(net,t,x,y,collocBd,collocIC,collocInt, ...
    ...
    % ---------------- Total loss ----------------
    loss = lossPDE + 5*lossIC + 2*lossData + 5*lossBC;
...
Need to calculcate the PDE, BC, DATA and IC's over the period in time.