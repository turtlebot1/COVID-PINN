...
I_true(1,round(Nx/2),round(Ny/2)) = 1; % infection bump at center
dt = 0.1;
for k = 2:Nt
...
for epoch = 1:numEpochs
    ...
    % Update monitor
    % recordMetrics(monitor,epoch,Loss=double(losses));
...
%     lambdaS = 5.0; lambdaI = 5.0; lambdaR = 2.0;
...
% 
%     % ---------------- Total loss ----------------
%     lossPDEn  = lossPDE  / (extractdata(lossPDE)  + 1e-6);
%     lossICn   = lossIC   / (extractdata(lossIC)   + 1e-6);
%     lossBCn   = lossBC   / (extractdata(lossBC)   + 1e-6);
%     lossDatan = lossData / (extractdata(lossData) + 1e-6);
% 
%     loss = wPDE*lossPDEn + lossBCn + 5*lossICn + lossDatan;
...
%             epoch, extractdata(lossPDEn), extractdata(lossBCn), ...
%             extractdata(lossICn), extractdata(lossDatan));
% 
%     % Gradients
%     grads = dlgradient(loss,net.Learnables);
% end
...
function [losses, grads, Gnorms, Ybd, Yic, S0] = modelLossGradNorm(net,t,x,y, ...
                                  collocBd,collocIC, ...
                                  beta,gamma,D_S,D_I,D_R, ...
                                  Xdata,Sdata,Idata,Rdata, ...
                                  wPDE,wIC,wBC,wData, collocInt)
