...
function [loss,grads] = modelLoss(net,t,x,y,collocBd,collocIC, ...
    ...
    loss = 10*lossPDE + lossBC + 5*lossIC;
    disp(loss);
...
%     Yic = forward(net,Xic);
% 
%     S0 = 50 * ones(size(Yic(1,:)),'like',Yic);   % susceptibles
%     Nx = 10; Ny = 10;
%     x_coords = Xic(2,:);  y_coords = Xic(3,:);
%     [~,centerIdx] = min((x_coords-0.5).^2 + (y_coords-0.5).^2);
%     I0 = zeros(size(Yic(2,:)),'like',Yic);  I0(centerIdx) = 50;
%     R0 = zeros(size(Yic(3,:)),'like',Yic);
% 
%     lossIC = mse(Yic(1,:), S0) ...
%        + 5*mse(Yic(2,:), I0) ...
%        + mse(Yic(3,:), R0);
...
%     lambdaS = 1.0; lambdaI = 5.0; lambdaR = 2.0;
...
% 
%     % ---------------- Total loss ----------------
%     loss = wPDE*lossPDE + lossBC + 5*lossIC + lossData;
% 
